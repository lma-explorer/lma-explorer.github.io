...
- **Third-party compilations of public-domain data** — e.g., the Livestock Marketing Information Center's (LMIC) compiled auction archives. Even though the underlying USDA-AMS facts are public-domain, a third-party compilation represents that party's editorial work and is typically subject to their membership or redistribution terms. This repository sources its auction data *directly* from USDA-AMS Market News (MARS API) rather than republishing any third-party compilation. Local research files derived from such compilations are kept outside the repository (see `.gitignore`).
