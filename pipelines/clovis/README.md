...
## Why USDA-AMS directly (not LMIC)

Historically, `Data_LMIC/AuctionsClovisNM.xlsx` (sheet `A1`) has been the go-to compilation for this series, going back to 1992. LMIC is member-access only, and their compilation is not redistributable on a public repo even though the underlying facts come from public-domain USDA reports. This pipeline replaces the LMIC path with a direct pull from USDA-AMS MARS, which is free, redistributable, and auto-refreshable. `Data_LMIC/` is kept locally as a research reference; it is gitignored.
